...
function CalendarMonth({ month, eventsByDay, selectedDay, onSelectDay, locale = "ru" }: CalendarMonthProps): JSX.Element {
            ...
            >
              ...
              <div className="calendar-month-events">
                {events.slice(0, 2).map((evt) => (
                  <span key={String(evt.id)} className={`calendar-month-dot dot-${String(evt.status || "").toLowerCase()}`} />
                ))}
