...
export interface CalendarDayProps {
  ...
  onJoin?: (evt: BookingDTO) => void;
  locale?: string;
}

interface DayEvent extends BookingDTO {
  _top: number;
  _height: number;
  _start: Date;
  _end: Date;
...
function CalendarDay({ day, events, onSelectEvent, onJoin, locale = "ru" }: CalendarDayProps): JSX.Element {
  ...
  const dayEvents = useMemo<DayEvent[]>(() => {
    return events
      .filter((evt) => Boolean(evt.start_at && evt.end_at))
      .map((evt) => {
        const s = new Date(evt.start_at as string);
        const e = new Date(evt.end_at as string);
