...
export default function OtpInput({ value = "", onChange, onComplete, disabled = false, error = false, autoFocus = true }: OtpInputProps): JSX.Element {
  ...
  const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement>, index: number): void => {
    if (disabled) return;
    ...
    if (newValue.length === OTP_LENGTH && onComplete) {
      window.setTimeout(() => onComplete(newValue), 0);
    }
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent<HTMLInputElement>): void => {
    ...
    if (pasted) {
      ...
      if (pasted.length === OTP_LENGTH && onComplete) {
        window.setTimeout(() => onComplete(pasted), 0);
      }
    }
