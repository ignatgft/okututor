 ...
 */
export function formatCountdown(countdown: CountdownResult, t: (key: string, defaultValue: string, options?: Record<string, unknown>) => string): string {
  if (countdown.isPast) {
  ...
  if (countdown.hours > 0) parts.push(`${countdown.hours} ${unit("plural.hour", "ч", countdown.hours)}`);
  if (countdown.minutes > 0) parts.push(`${countdown.minutes} ${unit("plural.minute", "мин", countdown.minutes)}`);
  if (parts.length === 0) parts.push(`${countdown.seconds} ${unit("plural.second", "с", countdown.seconds)}`);
