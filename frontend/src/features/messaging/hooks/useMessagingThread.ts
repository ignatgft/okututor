...
export function useMessagingThread(activeConvo: ConversationDTO | null): {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { if (document.hidden) return; void loadThread(); }, POLL_INTERVAL);
    const onVis = () => { if (!document.hidden) void loadThread(); };
    document.addEventListener("visibilitychange", onVis);
    return () => { cancelled = true; window.clearInterval(timer); document.removeEventListener("visibilitychange", onVis); };
  }, [activeConvo]);
