...
export const tutorApi = {
  ...
  /** By slug or id — прямая проверка /tutors/{slug} (поддерживает кириллицу), fallback на /tutors/slug/ */
  bySlug: async (slug: string | number): Promise<HttpResult<unknown>> => {
    const enc = encodeURIComponent(String(slug));
    try {
      const res = await apiClient.get<unknown>(`/api/v1/tutors/${enc}`, false);
      if (res.response.ok) return res;
    } catch {
      // ignore
    }
    try {
      const res2 = await apiClient.get<unknown>(`/api/v1/tutors/slug/${enc}`, false);
      if (res2.response.ok) return res2;
    } catch {
      // ignore
    }
    // последний fallback — без энкода (для совместимости)
    return apiClient.get<unknown>(`/api/v1/tutors/${String(slug)}`, false);
  },
