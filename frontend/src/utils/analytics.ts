 ...
 *
 * Privacy: uses Google Consent Mode v2. analytics_storage is default=denied
 * unless the visitor accepted analytics cookies in the cookie banner
 * (localStorage key "okututor_cookie_consent_v1" -> { all: true }).
 *
...
const CONSENT_KEY = "okututor_cookie_consent_v1";
...
/** Read the analytics consent decision previously stored by the cookie banner. */
export function hasAnalyticsConsent(): boolean {
  try {
    const raw = localStorage?.getItem(CONSENT_KEY);
    if (!raw) return false;
    const parsed = JSON.parse(raw) as { all?: boolean };
...
/** Update ga('consent') — must run before/after gtag is loaded. */
export function setAnalyticsConsent(granted: boolean): void {
  const windowWithDataLayer = window as unknown as { dataLayer?: unknown[]; gtag?: (...args: unknown[]) => void };
  const dataLayer = windowWithDataLayer.dataLayer ?? (windowWithDataLayer.dataLayer = []);
...
export function initAnalytics(): void {
  ...
  // Consent Mode v2 — default deny, upgrade from stored banner decision.
