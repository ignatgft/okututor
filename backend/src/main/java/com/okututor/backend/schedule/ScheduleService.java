...
public class ScheduleService {
    ...
    public Generated generateLessons(Schedule schedule) {
        return metrics.timedScheduleGeneration(() -> generateLessonsInternal(schedule));
    }

    private Generated generateLessonsInternal(Schedule schedule) {
        boolean doubleBookingPrevented = false;
        ...
        List<Lesson.Status> activeLessons = List.of(Lesson.Status.SCHEDULED, Lesson.Status.IN_PROGRESS);
        Instant now = Instant.now();
        int requested = schedule.getSlots().size();
        ...
        if (!bookings.isEmpty()) {
            ...
            metrics.scheduleGeneration(bookings.size() + conflicted.size(), savedBookings.size(),
                    conflicted.size(), doubleBookingPrevented);
        ...
        metrics.scheduleGeneration(requested, 0, conflicted.size(), doubleBookingPrevented);
