...
public class ObservabilityMetrics {
    ...
    private final AtomicLong pendingModerationGauge = new AtomicLong(0);
    private final AtomicLong resumesTotalGauge = new AtomicLong(0);

    public ObservabilityMetrics(MeterRegistry registry) {
        this.registry = registry;
        Gauge.builder("okututor_resumes_pending_moderation", pendingModerationGauge, AtomicLong::get)
                .description("Resumes pending moderation (Gauge)")
                .register(registry);
        Gauge.builder("okututor_moderation_pending_resumes", pendingModerationGauge, AtomicLong::get)
                .description("Pending moderation resumes")
                .register(registry);
        Gauge.builder("okututor_resumes_total", resumesTotalGauge, AtomicLong::get)
    ...
    public void authLoginFailed() {
        ...
        counter("okututor_auth_login_failure_total").increment();
    }

    public void authRefreshSuccess() {
        counter("okututor_auth_refresh_success_total").increment();
        counter("okututor_auth_refresh_total").increment();
    }

    public void authRefreshFailed() {
        counter("okututor_auth_refresh_failed_total").increment();
        counter("okututor_auth_refresh_failure_total").increment();
    }

    public void oauthSuccess() {
        counter("okututor_oauth_success_total").increment();
    }

    public void oauthFailure() {
        counter("okututor_oauth_failure_total").increment();
    }

    public void moderationApproved() {
        counter("okututor_moderation_approved_total").increment();
    }

    public void moderationRejected() {
        counter("okututor_moderation_rejected_total").increment();
    }

    public <T> T timedModeration(java.util.function.Supplier<T> action) {
        return timer("okututor_moderation_processing_seconds").record(action);
    }

    public void recordModerationDuration(long millis) {
        Timer.builder("okututor_moderation_processing_seconds").register(registry).record(millis, TimeUnit.MILLISECONDS);
    }
    ...
    // ---------------- marketplace tutor / resumes (spec 11) ----------------

    public void tutorProfileCreated() {
        counter("okututor_tutor_profile_created_total").increment();
        // okututor_resumes_total is Gauge (not Counter) — increment gauge only to avoid duplicate metric family
        resumesTotalGauge.incrementAndGet();
    }
    ...
    public void tutorProfilePublished() {
        ...
        counter("okututor_resumes_approved_total").increment();
        counter("okututor_moderation_approved_total").increment();
    }
    ...
    public void resumeRejected() {
        counter("okututor_resumes_rejected_total").increment();
        counter("okututor_moderation_rejected_total").increment();
    }

    public void pendingModerationInc() {
        pendingModerationGauge.incrementAndGet();
    }

    public void pendingModerationDec() {
        pendingModerationGauge.decrementAndGet();
    }

    public void setPendingModeration(long value) {
        pendingModerationGauge.set(value);
    }

    public void tutorProfileView() {
        counter("okututor_tutor_profile_view_total").increment();
        counter("okututor_resume_views_total").increment();
    }
    ...
    public void contactClick() {
        counter("okututor_contact_clicks_total").increment();
    }

    public void conversationCreated() {
        counter("okututor_conversations_total").increment();
    }

    public void messageSent() {
        counter("okututor_messages_total").increment();
    }

    public void userRegistered() {
        counter("okututor_users_registered_total").increment();
    }
