...
public class AdminController {
    ...
    private final com.okututor.backend.tutor.TutorProfileRepository tutorProfileRepository;
    private final com.okututor.backend.request.TutorRequestRepository tutorRequestRepository;
    ...
    public AdminController(CourseService courseService,
                           ...
                           com.okututor.backend.tutor.TutorProfileRepository tutorProfileRepository,
                           com.okututor.backend.request.TutorRequestRepository tutorRequestRepository,
    ...
    public Map<String, Long> stats() {
        return Map.of(
                ...
                "total_tutor_profiles", tutorProfileRepository.count(),
                "pending_tutor_profiles", tutorProfileRepository.countByStatus(
                        com.okututor.backend.tutor.TutorProfileStatus.PENDING_MODERATION),
                "published_tutor_profiles", tutorProfileRepository.countByStatus(
                        com.okututor.backend.tutor.TutorProfileStatus.PUBLISHED),
                "total_tutor_requests", tutorRequestRepository.count());
    }

    // ---------- заявки репетиторов (legacy TutorApplication, DEPRECATED: use /tutor-profiles; moved from /admin/tutors to avoid collision with TutorProfile moderation) ----------

    @GetMapping("/api/v1/admin/tutor-applications")
    ...
    @GetMapping("/api/v1/admin/tutor-applications/{id}")
    ...
    @PostMapping("/api/v1/admin/tutor-applications/{id}/approve")
    ...
    @PostMapping("/api/v1/admin/tutor-applications/{id}/reject")
    ...
    public Map<String, Object> metricsOverview() {
        // Legacy EDTECH counts kept for compatibility, plus marketplace core
        java.util.Map<String, Object> result = new java.util.LinkedHashMap<>();
        result.put("total_users", userRepository.count());
        result.put("total_tutors", tutorProfileRepository.countByStatus(com.okututor.backend.tutor.TutorProfileStatus.PUBLISHED));
        result.put("pending_tutor_profiles", tutorProfileRepository.countByStatus(com.okututor.backend.tutor.TutorProfileStatus.PENDING_MODERATION));
        result.put("rejected_tutor_profiles", tutorProfileRepository.countByStatus(com.okututor.backend.tutor.TutorProfileStatus.REJECTED));
        result.put("suspended_tutor_profiles", tutorProfileRepository.countByStatus(com.okututor.backend.tutor.TutorProfileStatus.SUSPENDED));
        result.put("total_tutor_requests", tutorRequestRepository.count());
        result.put("total_courses", courseService.countAll());
        result.put("total_bookings", bookingRepository.count());
        result.put("total_lessons", lessonRepository.count());
        result.put("total_reviews", reviewRepository.count());
        result.put("total_revenue", bookingRepository.revenueCompleted());
        result.put("pending_tutor_applications", tutorApplicationRepository.countByStatus(
                com.okututor.backend.tutors.TutorApplication.Status.PENDING));
        result.put("pending_enrollments", enrollmentRepository.countByStatus(
                com.okututor.backend.enrollment.Enrollment.Status.PENDING));
